...
| `js/app.js` | new | `FM.jobBegin()` / `FM.jobEnd()` / `FM.jobDepth()`: a counter with a 20 s watchdog that force-clears and writes a diagnostics report. | S0 |
| `js/app.js` | job bracketing | `try { FM.jobBegin(); … } finally { FM.jobEnd(); }` around the bodies of `duplicateLayer` (:3558), `duplicateSelection` (:3612), `pasteClipboard` (:3678), `splitLayer` (:4000), `replaceMediaWith` (≈:3808), `templates.insertInto`, `elements.insert` (storage.js), and ai-chat `applyTurn` (ai-chat.js:121). | S0 |
