...
(function (FM) {
  ...
  FM.EFFECTS = [
    ...
    { type: 'zoomblur', label: 'Zoom Blur', params: [
      ...
      { key: 'centerx', label: 'Centre X', min: 0, max: 100, step: 1, def: 50, unit: '%' },
      { key: 'centery', label: 'Centre Y', min: 0, max: 100, step: 1, def: 50, unit: '%' },
      { key: 'samples', label: 'Quality', min: 4, max: 32, step: 1, def: 9, unit: ' taps' },
    ] },
    ...
    { type: 'checker', label: 'Checker', color: true, defColor: '#000000', colorLabel: 'Colour', params: [
      { key: 'size', label: 'Cell size', min: 2, max: 120, step: 1, def: 24, unit: 'px' },
      { key: 'mix', label: 'Strength', min: 0, max: 1, step: 0.02, def: 0.5 },
      { key: 'ratio', label: 'Cell shape', min: 0.1, max: 4, step: 0.1, def: 1, unit: '×' },
      { key: 'angle', label: 'Angle', min: 0, max: 360, step: 1, def: 0, unit: '°' },
    ] },
    { type: 'grid', label: 'Grid', color: true, defColor: '#ffffff', colorLabel: 'Colour', params: [
      { key: 'size', label: 'Spacing', min: 4, max: 160, step: 1, def: 32, unit: 'px' },
      { key: 'thickness', label: 'Line weight', min: 1, max: 50, step: 1, def: 6, unit: '%' },
      { key: 'mix', label: 'Strength', min: 0, max: 1, step: 0.02, def: 1 },
      { key: 'angle', label: 'Angle', min: 0, max: 360, step: 1, def: 0, unit: '°' },
    ] },
    // ---- batch 7 ----
    { type: 'mosaic', label: 'Mosaic', params: [
      { key: 'size', label: 'Block size', min: 2, max: 100, step: 1, def: 16, unit: 'px' },
      { key: 'aspect', label: 'Block height', min: 25, max: 400, step: 5, def: 100, unit: '%' },
      { key: 'gap', label: 'Gap', min: 0, max: 50, step: 1, def: 0, unit: '%' },
      { key: 'sample', label: 'Colour from', def: 0, options: [[0, 'Average'], [1, 'Centre pixel']] },
    ] },
    { type: 'lensblur', label: 'Lens Blur', params: [
      { key: 'radius', label: 'Radius', min: 0, max: 30, step: 1, def: 10, unit: 'px' },
      { key: 'bloom', label: 'Highlight bloom', min: 0, max: 300, step: 5, def: 0, unit: '%' },
      { key: 'samples', label: 'Quality', min: 8, max: 32, step: 1, def: 16, unit: ' taps' },
      { key: 'blades', label: 'Aperture', def: 0, options: [[0, 'Circle'], [1, 'Hexagon'], [2, 'Pentagon'], [3, 'Square']] },
    ] },
    { type: 'dots', label: 'Dots', color: true, defColor: '#ffffff', colorLabel: 'Colour', params: [
      { key: 'size', label: 'Spacing', min: 4, max: 80, step: 1, def: 16, unit: 'px' },
      { key: 'radius', label: 'Dot size', min: 0.05, max: 0.7, step: 0.01, def: 0.32 },
      { key: 'opacity', label: 'Strength', min: 0, max: 1, step: 0.02, def: 0.85 },
      { key: 'softness', label: 'Edge softness', min: 0, max: 8, step: 0.5, def: 0, unit: 'px' },
    ] },
    ...
    { type: 'halftonelines', label: 'Halftone Lines', params: [
      ...
      { key: 'weight', label: 'Ink weight', min: 0.2, max: 2.5, step: 0.05, def: 1, unit: '×' },
      { key: 'softness', label: 'Edge softness', min: 0, max: 4, step: 0.1, def: 0, unit: 'px' },
    ] },
    { type: 'clouds', label: 'Clouds', color: true, defColor: '#ffffff', colorLabel: 'Cloud', params: [
      { key: 'amount', label: 'Amount', min: 0, max: 1, step: 0.02, def: 0.6 },
    ...
    { type: 'stripes', label: 'Stripes', color: true, defColor: '#000000', colorLabel: 'Colour', params: [
      { key: 'size', label: 'Width', min: 4, max: 80, step: 1, def: 16, unit: 'px' },
      { key: 'direction', label: 'Direction', def: 0, options: [[0, 'Diag ↘'], [1, 'Diag ↗'], [2, 'Across'], [3, 'Down']] },
      { key: 'duty', label: 'Bar / gap', min: 0.05, max: 0.95, step: 0.05, def: 0.5 },
      { key: 'strength', label: 'Strength', min: 0, max: 1, step: 0.02, def: 0.6 },
    ] },
    // ---- batch 9 ----
    ...
    { type: 'crosshatch', label: 'Crosshatch', color: true, defColor: '#101014', colorLabel: 'Ink', params: [
      ...
      { key: 'weight', label: 'Stroke weight', min: 1, max: 5, step: 1, def: 1, unit: 'px' },
    ...
    { type: 'sketch', label: 'Pencil Sketch', params: [
      ...
      { key: 'threshold', label: 'Clear smooth areas', min: 0, max: 100, step: 1, def: 0, unit: '%' },
      { key: 'tooth', label: 'Paper tooth', min: 0, max: 100, step: 1, def: 0, unit: '%' },
    ] },
  ...
  function applyParentChain(ctx, layer, t, scene) {
    ...
    while (pid && !seen.has(pid)) {
      seen.add(pid);
      const pl = FM.clipAt(scene, pid, t);   // the half that actually covers t, if this parent was split
      if (!pl) break;
      chain.push(pl);
      pid = pl.parent;
    }
  ...
  function drawFeatheredMaskLayer(ctx, layer, t, scene) {
    ...
    ctx.filter = 'none';
    ctx.drawImage(off, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(off,0,0) at scale 1
    ctx.restore();
  }
   ...
   * at project (0,0) — right whenever the pass is drawing into the comp, wrong whenever it is drawing
   * into something bigger.
   * SOMETHING BIGGER EXISTS, AND IT IS SQUISH. It renders the layer into a plate PADDED past every
   * wall, because the part hanging outside the frame is the part it squashes back in — then draws the
   * layer into that plate with all its OTHER effects applied, and each of those allocated a comp-sized
   * plate INSIDE the padded one and clipped the overhang away. Ezra: *"if a shake makes it move the
   * squish doesn't do anything"*.
   * The target already carries its extent and origin (`__fmOX`/`__fmOY`, project units, negative when
  ...
  /* Effects that move the WHOLE layer rather than rearranging pixels inside it (queue 323). Squish
  ...
  function drawPixelEffect(ctx, layer, t, scene, fx, fn) {
    ...
    try {
      ...
      pA.__fmRS = ps; pA.__fmOX = OX; pA.__fmOY = OY;   // the nested drawLayer renders through baseT, and a nested effect inherits this scale
      ...
      ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
      ctx.filter = 'none';
      ctx.drawImage(pB, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(pB,0,0) at scale 1
      ctx.restore();
    } finally { _pfDepth--; }
  }
  ...
  function drawFilterContainer(ctx, layer, t, scene, fx) {
    ...
    try {
      ...
      const plate = (cv, list) => {
        cv.__fmRS = ps; cv.__fmOX = OX; cv.__fmOY = OY;   // a nested effect inherits this scale
        const c = cv.getContext('2d');
        baseT(c); c.clearRect(OX, OY, PWp, PHp);
      ...
      ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
      ctx.filter = 'none';
      ctx.drawImage(P.M, OX, OY, PWp, PHp);
      ctx.restore();
    } finally { _fcDepth--; }
  }
  ...
  /* ONE REUSED SCRATCH FOR THE "copy the frame, then read it while you overwrite it" PATTERN
     ...
     MEASURED at that size: `slice()` costs 1.59ms; copying into a buffer that already exists costs
     **0.24ms**. Over the 24 effects in Ezra's own slow reading that is 32ms of the 294 — but the bigger
     number is the garbage: **133 MB of throwaway allocation per frame**, which on a phone is collected
     in pauses, and a pause is exactly what "playback is a buggy mess while scrubbing is fine" feels like.
     SAFE because every one of the sixteen only ever READS its copy — checked, not assumed: none of them
     writes into it, and the kernels run one after another, never nested. Kernels that keep two or three
     copies (boxblur, unsharpmask, sketch) are deliberately left alone; one shared buffer cannot serve two
     live copies at once and quietly returning the same array to both would corrupt the picture.
     The buffer only ever GROWS, so alternating plate sizes cannot thrash it: `set` writes d.length bytes
     at offset 0 and nothing reads past that. */
  let _fxScratch = null;
  function fxSrc(d) {
    if (!_fxScratch || _fxScratch.length < d.length) _fxScratch = new Uint8ClampedArray(d.length);
    _fxScratch.set(d);
    return _fxScratch;
  }
  ...
  const PIXEL_FX = {
    ...
    shockwave: function (d, W, H, p, t, ps) {
      ...
      const src = fxSrc(d), R0s = R0 * R0, R1s = R1 * R1;
      for (let y = y0; y < y1; y++) {
    ...
    thermal: (function () {
      ...
      return function (d, W, H, p, t) {
        ...
        /* TWO SLIDERS THAT CAN CROSS (queue 261's sibling, 262). Nothing stops Low being dragged above
         * High, and the arithmetic below did not survive it: at Low == High the span is 0 and the old
         * `span === 0 ? 0` guard mapped EVERY luminance to ramp position 0 — black in four of the six
         * palettes — while a negative span inverted and clamped to one end. Either way the layer became
         * a flat rectangle of one colour, which on a dark canvas reads as "my layer disappeared", not
         * as "I crossed two sliders". Measured on rgb(53,220,175): 50/50 gave flat black, 80/20 gave
         * flat rgb(40,0,141) with every pixel identical.
         * Crossed sliders are now read as the WINDOW they describe (swap), and a zero-width window is
         * widened to the narrowest one that still maps a range. Both keep the layer legible instead of
         * erasing it, and neither changes any Low < High setting that already worked. */
        let lo = p.low == null ? 0 : FM.evalProp(p.low, t), hi = p.high == null ? 100 : FM.evalProp(p.high, t);
        if (!isFinite(lo)) lo = 0; if (!isFinite(hi)) hi = 100;
        if (hi < lo) { const sw = lo; lo = hi; hi = sw; }
        if (hi - lo < 1) { hi = Math.min(100, lo + 1); lo = Math.max(0, hi - 1); }
    ...
    halftone: function (d, W, H, p, t, ps) {
      const size = Math.max(2, Math.round((FM.evalProp(p.size, t) || 8) * (ps || 1))), r2 = size / 2, s = fxSrc(d);
      // ANGLE is the one that matters: every real print halftone runs its screen at 45 degrees,
      // because an axis-aligned dot grid beats against the subject and reads as a moire artefact
      // rather than as print. GAIN was a hardcoded 1.45 — ink coverage, the difference between a
      // newsprint smudge and a crisp screen. SHAPE was always a circle.
      const ang = p.angle == null ? 0 : FM.evalProp(p.angle, t);
      const rot = ((ang % 360) + 360) % 360 !== 0;
      const rad = ang * Math.PI / 180, ca = Math.cos(rad), sa = Math.sin(rad);
    ...
    zoomblur: function (d, W, H, p, t) {
      ...
      // CENTRE was W/2, H/2 — the rush could only ever come out of the middle of the frame, so it
      // could not be aimed at the subject it is supposed to be rushing toward.
      const s = fxSrc(d), cx = wCx(p, t, W, W / 2), cy = wCy(p, t, H, H / 2);
      const N = p.samples == null ? 9 : Math.max(4, Math.min(32, Math.round(FM.evalProp(p.samples, t))));
      // SAMPLES is a quality knob and must not change the REACH, or turning it up would also lengthen
      // the streak. Legacy walks k/N for k<N, so its furthest tap is 8/9 of amt*0.35, not amt*0.35 —
      // that 8/9 is what the general form has to reproduce. At N === 9 the literal legacy expression is
    ...
    checker: function(d,W,H,p,t,ps){ var chkSz=FM.evalProp(p.size,t); chkSz=(chkSz==null?24:chkSz); chkSz=Math.max(2,Math.min(120,Math.round(chkSz))); chkSz=Math.max(1,Math.round(chkSz*(ps||1))); /* px pattern period — x ps so a reduced preview plate matches the export, as halftone already does */  var chkCol=hexToRGB(p.color)||[0,0,0]; var chkR=chkCol[0],chkG=chkCol[1],chkB=chkCol[2];
      ...
      // RATIO frees the cell from being square, ANGLE from being axis-aligned. Half a step is exact in
      // binary, so a mix of 0.5 through the lerp is the same byte as the original (a+b)*0.5.
      var chkMix=p.mix==null?0.5:FM.evalProp(p.mix,t); if(chkMix<0)chkMix=0; if(chkMix>1)chkMix=1; if(chkMix<=0)return;
      var chkRatio=p.ratio==null?1:FM.evalProp(p.ratio,t); if(chkRatio<0.1)chkRatio=0.1; if(chkRatio>4)chkRatio=4;
    ...
    grid: function(d,W,H,p,t){ var grSize=FM.evalProp(p.size,t); grSize=(grSize==null?32:grSize); grSize=Math.round(grSize); if(grSize<4)grSize=4; if(grSize>160)grSize=160;
      ...
      var grThP=p.thickness==null?6:FM.evalProp(p.thickness,t); if(grThP<1)grThP=1; if(grThP>50)grThP=50;
      var grLW=Math.max(1,Math.round(grThP===6?grSize*0.06:grSize*(grThP/100)));
      var grMix=p.mix==null?1:FM.evalProp(p.mix,t); if(grMix<0)grMix=0; if(grMix>1)grMix=1; if(grMix<=0)return;
      var grAng=p.angle==null?0:FM.evalProp(p.angle,t); var grRot=((grAng%360)+360)%360!==0;
    ...
    mosaic: function(d,W,H,p,t,ps){ var moBs=Math.round(FM.evalProp(p.size,t)||16); if(moBs<2)moBs=2; if(moBs>100)moBs=100; moBs=Math.max(2,Math.round(moBs*(ps||1)));   /* block size is PROJECT px — see plateScale */ var moS=fxSrc(d),moW4=W*4;
      /* Square, averaged, gapless blocks and nothing else — no wide scanline cells, no tile grid, and
         no crisp nearest-neighbour retro sampling. ASPECT frees the cell's height from its width;
         GAP leaves the original showing between tiles; SAMPLE Centre takes the middle pixel instead of
         the mean, which is what gives a hard retro pixel look rather than a soft averaged one. */
      var moAsp=p.aspect==null?100:FM.evalProp(p.aspect,t); if(moAsp<25)moAsp=25; if(moAsp>400)moAsp=400;
      var moGap=p.gap==null?0:FM.evalProp(p.gap,t); if(moGap<0)moGap=0; if(moGap>50)moGap=50;
      var moCen=(p.sample==null?0:(Math.round(FM.evalProp(p.sample,t))|0))===1;
      var moBsY=moAsp===100?moBs:Math.max(2,Math.round(moBs*(moAsp/100)));
      var moIn=moGap===0?0:Math.round(moBs*(moGap/100)*0.5); for(var moBy=0;moBy<H;moBy+=moBsY){ var moY1=Math.min(moBy+moBsY,H); for(var moBx=0;moBx<W;moBx+=moBs){ var moX1=Math.min(moBx+moBs,W),moSr=0,moSg=0,moSb=0,moSa=0,moN=0; for(var moY=moBy;moY<moY1;moY++){ var moRow=moY*moW4; for(var moX=moBx;moX<moX1;moX++){ var moI=moRow+moX*4; moSr+=moS[moI]; moSg+=moS[moI+1]; moSb+=moS[moI+2]; moSa+=moS[moI+3]; moN++; } } if(moN===0)continue;
        var moAr,moAg,moAb,moAa;
        if(moCen){ var moCx=Math.min(W-1,moBx+((moX1-moBx)>>1)), moCy=Math.min(H-1,moBy+((moY1-moBy)>>1)), moC=(moCy*W+moCx)*4;
          moAr=moS[moC]; moAg=moS[moC+1]; moAb=moS[moC+2]; moAa=moS[moC+3]; }
        else { moAr=moSr/moN; moAg=moSg/moN; moAb=moSb/moN; moAa=moSa/moN; }
        var moY0=moBy+moIn, moYe=moY1-moIn, moX0=moBx+moIn, moXe=moX1-moIn;   // gap: leave a margin untouched
        for(var moY2=moY0;moY2<moYe;moY2++){ var moRow2=moY2*moW4; for(var moX2=moX0;moX2<moXe;moX2++){ var moJ=moRow2+moX2*4; d[moJ]=moAr; d[moJ+1]=moAg; d[moJ+2]=moAb; d[moJ+3]=moAa; } } } } },
    lensblur: function(d,W,H,p,t){ var lb_r=FM.evalProp(p.radius,t); lb_r=(lb_r==null?10:lb_r); if(lb_r<0)lb_r=0; if(lb_r>30)lb_r=30; if(lb_r<1)return;
      // Three things this effect could not do, in the order they matter.
      // BLOOM is the real one. A flat average of sRGB takes a specular's brightness, spreads it over
      // 16 taps and divides by 16 — so the highlight DISSOLVES and the result is a slightly nicer Box
      // Blur. Weighting each tap by how bright it is lets a point of light dominate its own disc,
      // which is the whole difference between defocus and bokeh. The shipped description has always
      // said "bright points bloom into the aperture shape"; until now that sentence was not true.
      // SAMPLES: 16 taps across a 30px disc is one tap per ~177px^2 — a sparse spray that reads as
      // noise rather than as a lens. BLADES: the aperture was always a perfect circle.
      // SAMPLES tops out at 32, not the 64 the plan proposed. Measured at export resolution on a
      // 1080x1920 plate at radius 30 with bloom: 16 taps 372ms, 32 taps 622ms, 64 taps 1217ms. A frame
      // that takes 1.2s is a trap on a phone, and the density gain past 32 is not visible — 32 already
      // doubles the tap count that made the disc read as a sparse spray, which was the actual complaint.
      var lb_n=p.samples==null?16:Math.max(8,Math.min(32,Math.round(FM.evalProp(p.samples,t))));
      var lb_bl=p.blades==null?0:(Math.round(FM.evalProp(p.blades,t))|0);
      var lb_sides=lb_bl===1?6:(lb_bl===2?5:(lb_bl===3?4:0));
      var lb_bloomP=p.bloom==null?0:FM.evalProp(p.bloom,t); if(lb_bloomP<0)lb_bloomP=0; if(lb_bloomP>300)lb_bloomP=300;
      var lb_bloom=lb_bloomP/100, lb_glow=lb_bloomP>0;
      var lb_s=fxSrc(d),lb_w4=W*4,lb_ox=new Float64Array(lb_n),lb_oy=new Float64Array(lb_n),lb_k;
      // A pixel's bloom weight depends only on that pixel, so computing it inside the tap loop worked
      // it out once per tap that LANDS on it — N times over, for every pixel in the frame. Lifted to a
      // single pass it is one array read per tap instead of a luma dot product and two multiplies.
      var lb_wt=null;
      if(lb_glow){ lb_wt=new Float32Array(W*H);
        for(var lb_q=0,lb_pi=0;lb_pi<lb_wt.length;lb_q+=4,lb_pi++){
          var lb_ll=(lb_s[lb_q]*0.299+lb_s[lb_q+1]*0.587+lb_s[lb_q+2]*0.114)/255;
          var lb_hh=lb_ll>0.6?(lb_ll-0.6)*2.5:0;
          lb_wt[lb_pi]=1+lb_bloom*lb_hh*lb_hh; } }
      // The aperture is baked into the tap offsets, so a polygon costs nothing per pixel. The distance
      // from the centre to an n-gon's edge at angle a is apothem/cos(a within its own sector) — the
      // circumradius at a vertex, the apothem facing an edge. The disc is squeezed inward, never
      // stretched, so a hexagon can never sample further out than the circle it replaces.
      var lb_sec=lb_sides?(Math.PI*2/lb_sides):0, lb_ap=lb_sides?Math.cos(Math.PI/lb_sides):1;
      for(lb_k=0;lb_k<lb_n;lb_k++){ var lb_a=lb_k*2.399963,lb_rd=lb_r*Math.sqrt((lb_k+0.5)/lb_n);
        if(lb_sides){ var lb_lo=lb_a-Math.floor(lb_a/lb_sec)*lb_sec-lb_sec/2; lb_rd*=lb_ap/Math.cos(lb_lo); }
        lb_ox[lb_k]=Math.cos(lb_a)*lb_rd;lb_oy[lb_k]=Math.sin(lb_a)*lb_rd; }
      for(var lb_y=0;lb_y<H;lb_y++){for(var lb_x=0;lb_x<W;lb_x++){ var lb_sr=0,lb_sg=0,lb_sb=0,lb_sa=0,lb_ws=0;
        for(lb_k=0;lb_k<lb_n;lb_k++){ var lb_sx=lb_x+lb_ox[lb_k]|0,lb_sy=lb_y+lb_oy[lb_k]|0;
          ...
          var lb_si=lb_sy*lb_w4+lb_sx*4;
          if(lb_glow){ var lb_w=lb_wt[lb_si>>2];
            lb_sr+=lb_s[lb_si]*lb_w;lb_sg+=lb_s[lb_si+1]*lb_w;lb_sb+=lb_s[lb_si+2]*lb_w;lb_sa+=lb_s[lb_si+3]*lb_w;lb_ws+=lb_w; }
          else { lb_sr+=lb_s[lb_si];lb_sg+=lb_s[lb_si+1];lb_sb+=lb_s[lb_si+2];lb_sa+=lb_s[lb_si+3]; } }
        // Legacy divides by the literal 16; lb_n IS 16 on an un-upgraded instance, and with bloom off
        // the weighted branch is never entered, so both the taps and the divisor are bit-identical.
        var lb_di=lb_y*lb_w4+lb_x*4, lb_dv=lb_glow?lb_ws:lb_n;
        d[lb_di]=lb_sr/lb_dv;d[lb_di+1]=lb_sg/lb_dv;d[lb_di+2]=lb_sb/lb_dv;d[lb_di+3]=lb_sa/lb_dv;}} },
    dots: function(d,W,H,p,t,ps){ var dt_sz=FM.evalProp(p.size,t); if(dt_sz==null)dt_sz=16; dt_sz=Math.max(4,Math.min(80,dt_sz)); dt_sz=Math.max(2,dt_sz*(ps||1)); var dt_col=hexToRGB(p.color);
      /* Dot-to-cell ratio and opacity were both hardcoded, so the ONLY pattern available was "medium
         dots at 85%", with hard aliased edges. SOFTNESS feathers the rim over a few pixels, which is
         what stops a dot screen looking like a bitmap. */
      ...
      var dt_cr=dt_rP===0.32?dt_sz*0.32:dt_sz*dt_rP, dt_r2=dt_cr*dt_cr, dt_a=dt_aP, dt_ia=1-dt_a, dt_w4=W*4; for(var dt_y=0;dt_y<H;dt_y++){ var dt_dcy=dt_y-(Math.floor(dt_y/dt_sz)*dt_sz+dt_sz/2); var dt_row=dt_y*dt_w4; for(var dt_x=0;dt_x<W;dt_x++){ var dt_i=dt_row+dt_x*4; if(d[dt_i+3]===0)continue; var dt_dcx=dt_x-(Math.floor(dt_x/dt_sz)*dt_sz+dt_sz/2); var dt_q=dt_dcx*dt_dcx+dt_dcy*dt_dcy;
        if(dt_sf<=0){ if(dt_q<=dt_r2){ d[dt_i]=d[dt_i]*dt_ia+dt_col[0]*dt_a; d[dt_i+1]=d[dt_i+1]*dt_ia+dt_col[1]*dt_a; d[dt_i+2]=d[dt_i+2]*dt_ia+dt_col[2]*dt_a; } }
          ...
          var dt_k=dt_dd<=dt_cr?1:1-(dt_dd-dt_cr)/dt_sf; var dt_kk=dt_a*dt_k, dt_ik=1-dt_kk;
    ...
    halftonelines: function(d,W,H,p,t,ps){ var htlSize=FM.evalProp(p.size,t); if(htlSize==null||isNaN(htlSize))htlSize=8; htlSize=Math.max(3,Math.min(40,Math.round(htlSize))); /* FLOOR THE SCALED PERIOD AT 2, NOT 1 (queue 261). A stripe needs two rows: at a period of 1 every
       ...
       pixels, half res 24,300, and the 360-wide preview ZERO. Not a cosmetic difference: `ps` is moved
       by the QUALITY LADDER, so a halftone layer could turn into a black rectangle mid-playback while
      ...
      // step off the edge of every line.
      var htlAng=p.angle==null?0:FM.evalProp(p.angle,t);
      var htlRot=((htlAng%360)+360)%360!==0;
      ...
      for(var htlY=0;htlY<H;htlY++){ var htlRowMod=((htlY%htlSize)+htlSize)%htlSize; var htlRowBase=htlY*htlW4;
        for(var htlX=0;htlX<W;htlX++){ var htlI=htlRowBase+htlX*4; if(d[htlI+3]===0)continue;
          ...
          var htlM=htlRowMod;
          if(htlRot){ var htlV2=-htlX*htlSin+htlY*htlCos; htlM=htlV2%htlSize; if(htlM<0)htlM+=htlSize; }
          // THE CAP IS NOT COSMETIC — it is the queue-261 failure reachable through a new door. The
          // note above explains that once the threshold beats the period, EVERY row of every period is
          // inked and the layer is written solid black; `ps` shrinks the period on a reduced preview
          // plate, so at 1/3 scale the pitch here is 3px and a weight of 2.5 clears it for any tone
          // below 60% — the layer turns into a black rectangle mid-playback while the export is fine.
          // Caught by the queue-264 extremes sweep. Applied ONLY in the weighted branch: at weight 1
          // the legacy expression must stay bit-for-bit what it was, and it reaches the period only on
          // near-black tones, where solid ink is the right answer anyway.
          var htlThresh;
          if(htlWt===1) htlThresh=(1-htlL)*htlSize;
          else { htlThresh=(1-htlL)*htlSize*htlWt; var htlCap=htlSize-1; if(htlThresh>htlCap)htlThresh=htlCap; }
          var htlV;
    ...
    stripes: function(d,W,H,p,t,ps){ var stp_size=FM.evalProp(p.size,t); if(stp_size==null)stp_size=16; stp_size=Math.max(4,Math.min(80,stp_size)); var stp_period=Math.max(2,Math.round(stp_size*(ps||1))); /* px pattern period — x ps so a reduced preview plate matches the export, as halftone already does */  var stp_c=hexToRGB(p.color); var stp_r=stp_c[0],stp_g=stp_c[1],stp_b=stp_c[2];
      // Locked to a 45-degree diagonal at a 50% duty cycle mixed at a fixed 0.6 — the only thing you
      // could change was how WIDE they were. DIRECTION picks the axis, DUTY the bar-to-gap ratio,
      // STRENGTH how solid they are. All three defaults are the constants they replace.
      var stp_dir=p.direction==null?0:(Math.round(FM.evalProp(p.direction,t))|0);
      var stp_duty=p.duty==null?0.5:FM.evalProp(p.duty,t); if(stp_duty<0.05)stp_duty=0.05; if(stp_duty>0.95)stp_duty=0.95;
      var stp_half=stp_duty===0.5?stp_period*0.5:stp_period*stp_duty;
      var stp_k=p.strength==null?0.6:FM.evalProp(p.strength,t); if(stp_k<0)stp_k=0; if(stp_k>1)stp_k=1; if(stp_k<=0)return;
      var stp_ik=1-stp_k;
    ...
    // ---- batch 10 (pixel) ----
    bumpmap: function(d,W,H,p,t){ var bmAmt=FM.evalProp(p.amount,t); if(bmAmt==null)bmAmt=1.2; bmAmt=Math.max(0,Math.min(3,bmAmt)); var bmS=fxSrc(d); var bmW4=W*4;
    ...
    contourlines: function(d,W,H,p,t){ var clLv=Math.round(FM.evalProp(p.levels,t)||8); if(clLv<2)clLv=2; if(clLv>24)clLv=24; var clS=fxSrc(d),clW4=W*4,clScl=clLv/255; var clBand=new Int16Array(W*H);
    ...
    hextiles: function(d,W,H,p,t,ps){ var hxSize=FM.evalProp(p.size,t); if(hxSize==null)hxSize=20; if(hxSize<4)hxSize=4; if(hxSize>80)hxSize=80; hxSize=Math.max(2,hxSize*(ps||1));   /* tile size is PROJECT px — see plateScale */ var hxRowH=hxSize*0.75; if(hxRowH<1)hxRowH=1; var hxHalf=hxSize*0.5; var hxSrc=fxSrc(d); var hxW4=W*4; for(var hxY=0;hxY<H;hxY++){ var hxRow=Math.floor(hxY/hxRowH); var hxShift=(hxRow&1)?hxHalf:0; for(var hxX=0;hxX<W;hxX++){ var hxCol=Math.floor((hxX-hxShift)/hxSize); var hxBestDx=1e9,hxBestX=hxX,hxBestY=hxY; for(var hxRO=-1;hxRO<=1;hxRO++){ var hxR2=hxRow+hxRO; var hxCY=hxR2*hxRowH+hxRowH*0.5; var hxSh2=(hxR2&1)?hxHalf:0; for(var hxCO=-1;hxCO<=1;hxCO++){ var hxC2=hxCol+hxCO; var hxCX=hxC2*hxSize+hxSh2+hxHalf; var hxDX=hxX-hxCX, hxDY=hxY-hxCY; var hxDist=hxDX*hxDX+hxDY*hxDY; if(hxDist<hxBestDx){ hxBestDx=hxDist; hxBestX=Math.round(hxCX); hxBestY=Math.round(hxCY); } } } if(hxBestX<0)hxBestX=0; else if(hxBestX>=W)hxBestX=W-1; if(hxBestY<0)hxBestY=0; else if(hxBestY>=H)hxBestY=H-1; var hxSi=(hxBestY*W+hxBestX)*4; var hxDi=hxY*hxW4+hxX*4; d[hxDi]=hxSrc[hxSi]; d[hxDi+1]=hxSrc[hxSi+1]; d[hxDi+2]=hxSrc[hxSi+2]; d[hxDi+3]=hxSrc[hxSi+3]; } } },
    ...
    dissolve: function(d,W,H,p,t){ var dsAmt=FM.evalProp(p.amount,t); if(dsAmt==null)dsAmt=0.5; if(dsAmt<0)dsAmt=0; if(dsAmt>1)dsAmt=1; if(dsAmt<=0)return;
      /* One static salt-and-pepper pattern, identical in every project and every instance: it ate the
         ...
         threshold across the frame so the dissolve sweeps; SOFT gives the holes a partial edge; SPEED
         re-rolls the pattern over time so it boils instead of sitting still. */
      ...
      for(var dsY=0;dsY<H;dsY++){ for(var dsX=0;dsX<W;dsX++){ var dsI=(dsY*W+dsX)<<2; if(d[dsI+3]===0)continue;
        var dsH=(dsX*374761393+dsY*668265263+dsFr*2246822519)>>>0; dsH=(dsH^(dsH>>>13))>>>0; dsH=(dsH*1274126177)>>>0; dsH=(dsH^(dsH>>>16))>>>0;
        if(dsPlain){ if(dsH<dsThr)d[dsI+3]=0; continue; }
        /* A directional dissolve is the SAME noise with a ramped threshold: near the leading edge the
           bar is high so almost everything goes, far from it almost nothing does. */
        var dsA=dsAmt;
        /* dsU is DISTANCE FROM THE LEADING EDGE: 0 where the dissolve starts, 1 at the far side.
    ...
    hexarray: function(d,W,H,p,t){ var hx_s=FM.evalProp(p.size,t); if(hx_s==null) hx_s=24; if(hx_s<8) hx_s=8; if(hx_s>80) hx_s=80; var hx_col=hexToRGB(p.color||'#19d6c0'); var hx_cr=hx_col[0], hx_cg=hx_col[1], hx_cb=hx_col[2]; var hx_rh=hx_s*0.8660254;
      ...
      var hx_tP=p.thickness==null?0.12:FM.evalProp(p.thickness,t); if(hx_tP<0.02)hx_tP=0.02; if(hx_tP>0.5)hx_tP=0.5;
      var hx_op=p.opacity==null?1:FM.evalProp(p.opacity,t); if(hx_op<0)hx_op=0; if(hx_op>1)hx_op=1; if(hx_op<=0)return;
      var hx_band=Math.max(1, hx_tP===0.12?hx_s*0.12:hx_s*hx_tP); for(var hx_y=0; hx_y<H; hx_y++){ var hx_brow=Math.round(hx_y/hx_rh); for(var hx_x=0; hx_x<W; hx_x++){ var hx_idx=(hx_y*W+hx_x)*4; if(d[hx_idx+3]===0) continue; var hx_d1=1e20, hx_d2=1e20; for(var hx_dr=-1; hx_dr<=1; hx_dr++){ var hx_r=hx_brow+hx_dr; var hx_ox=(hx_r&1)?(hx_s*0.5):0; var hx_bc=Math.round((hx_x-hx_ox)/hx_s); for(var hx_dc=-1; hx_dc<=1; hx_dc++){ var hx_c=hx_bc+hx_dc; var hx_px=hx_c*hx_s+hx_ox; var hx_py=hx_r*hx_rh; var hx_ex=hx_x-hx_px, hx_ey=hx_y-hx_py; var hx_dist=Math.sqrt(hx_ex*hx_ex+hx_ey*hx_ey); if(hx_dist<hx_d1){ hx_d2=hx_d1; hx_d1=hx_dist; } else if(hx_dist<hx_d2){ hx_d2=hx_dist; } } } var hx_diff=hx_d2-hx_d1; if(hx_diff<hx_band){ var hx_a=1-hx_diff/hx_band; if(hx_a<0) hx_a=0; if(hx_a>1) hx_a=1; if(hx_op!==1) hx_a*=hx_op; var hx_ia=1-hx_a; d[hx_idx]=d[hx_idx]*hx_ia+hx_cr*hx_a; d[hx_idx+1]=d[hx_idx+1]*hx_ia+hx_cg*hx_a; d[hx_idx+2]=d[hx_idx+2]*hx_ia+hx_cb*hx_a; } } } },
    // ---- batch 17 (drawing / blur / proc) ----
    electricedges: function(d,W,H,p,t){ var eeAmt=FM.evalProp(p.amount,t); if(eeAmt==null)eeAmt=0.6; if(eeAmt<0)eeAmt=0; if(eeAmt>1)eeAmt=1; var eeSpd=FM.evalProp(p.speed,t); if(eeSpd==null)eeSpd=4; if(eeSpd<0)eeSpd=0; if(eeSpd>10)eeSpd=10; var eeCol=hexToRGB(p.color); var eeR=eeCol[0], eeG=eeCol[1], eeB=eeCol[2]; var ees=fxSrc(d); var eew4=W*4; var eeFrame=Math.floor(t*eeSpd); for(var eey=0;eey<H;eey++){ for(var eex=0;eex<W;eex++){ var eei=(eey*W+eex)*4; if(ees[eei+3]===0)continue; var eexm=eex>0?eex-1:0; var eexp=eex<W-1?eex+1:W-1; var eeym=eey>0?eey-1:0; var eeyp=eey<H-1?eey+1:H-1; var eeRow0=eeym*eew4, eeRow1=eey*eew4, eeRow2=eeyp*eew4; var eeXm4=eexm*4, eeX4=eex*4, eeXp4=eexp*4; var eeTL=ees[eeRow0+eeXm4]*0.299+ees[eeRow0+eeXm4+1]*0.587+ees[eeRow0+eeXm4+2]*0.114; var eeT=ees[eeRow0+eeX4]*0.299+ees[eeRow0+eeX4+1]*0.587+ees[eeRow0+eeX4+2]*0.114; var eeTR=ees[eeRow0+eeXp4]*0.299+ees[eeRow0+eeXp4+1]*0.587+ees[eeRow0+eeXp4+2]*0.114; var eeL=ees[eeRow1+eeXm4]*0.299+ees[eeRow1+eeXm4+1]*0.587+ees[eeRow1+eeXm4+2]*0.114; var eeRr=ees[eeRow1+eeXp4]*0.299+ees[eeRow1+eeXp4+1]*0.587+ees[eeRow1+eeXp4+2]*0.114; var eeBL=ees[eeRow2+eeXm4]*0.299+ees[eeRow2+eeXm4+1]*0.587+ees[eeRow2+eeXm4+2]*0.114; var eeBb=ees[eeRow2+eeX4]*0.299+ees[eeRow2+eeX4+1]*0.587+ees[eeRow2+eeX4+2]*0.114; var eeBR=ees[eeRow2+eeXp4]*0.299+ees[eeRow2+eeXp4+1]*0.587+ees[eeRow2+eeXp4+2]*0.114; var eeGx=(eeTR+2*eeRr+eeBR)-(eeTL+2*eeL+eeBL); var eeGy=(eeBL+2*eeBb+eeBR)-(eeTL+2*eeT+eeTR); var eeMag=Math.sqrt(eeGx*eeGx+eeGy*eeGy)/1442; if(eeMag<=0)continue; if(eeMag>1)eeMag=1; var eeH=(eex*374761393+eey*668265263+eeFrame*2147483647)>>>0; eeH=(eeH^(eeH>>>13))*1274126177>>>0; eeH=(eeH^(eeH>>>16))>>>0; var eeFlick=0.45+(eeH/4294967295)*0.55; /* THIS EFFECT WAS THE FAINTEST IN THE APP BY A WIDE MARGIN (queue 482, round 2). Sweeping all 105
   pixel effects at the settings they ship with and taking the 95th-percentile pixel change: the
   median effect scores 131 and the first quartile 60. Electric Edges scored **2** — five times below
   the next weakest and about sixty-five times below the median. Edges you cannot see are not an
   effect, they are a rumour.
   The cause is the normaliser: the Sobel magnitude is divided by 1442, the THEORETICAL maximum for a
   3x3 Sobel, which no real edge approaches — a hard 245-to-20 boundary only reaches ~0.62, and after
...
var eeAdd=eeMag*eeAmt*eeFlick*3.6; if(eeAdd<=0)continue; if(eeAdd>1)eeAdd=1; var eeSr=255-(255-d[eei])*(255-eeR*eeAdd)/255; var eeSg=255-(255-d[eei+1])*(255-eeG*eeAdd)/255; var eeSb=255-(255-d[eei+2])*(255-eeB*eeAdd)/255; d[eei]=eeSr; d[eei+1]=eeSg; d[eei+2]=eeSb; } } },
    ...
    crosshatch: function(d,W,H,p,t,ps){ var sp=FM.evalProp(p.spacing,t); if(sp==null)sp=7; sp=Math.round(sp); if(sp<3)sp=3; if(sp>30)sp=30; sp=Math.max(1,Math.round(sp*(ps||1))); /* px pattern period — x ps so a reduced preview plate matches the export, as halftone already does */  var col=hexToRGB(p.color); var ir=col[0],ig=col[1],ib=col[2]; var W4=W*4;
      ...
      // DENSITY scales all three tiers together (50 leaves them exactly where they were), WEIGHT is the
      // stroke width, ANGLE turns all three axes together so the hatch keeps its 45-degree relationships.
      ...
      for(var y=0;y<H;y++){ var row=y*W4; var ymod=y%sp;
        for(var x=0;x<W;x++){ var ci=row+x*4; var a=d[ci+3]; if(a===0)continue;
          ...
          var hx=x, hy=y, hym=ymod;
          if(chRot){ hx=x*chCos+y*chSin; hy=-x*chSin+y*chCos; hym=hy%sp; if(hym<0)hym+=sp; }
          if(l<chT1){ var xy=(hx+hy)%sp; if(xy<0)xy+=sp; if(xy<chWt)hatch=true; }
          if(!hatch&&l<chT2){ var xmy=(hx-hy)%sp; if(xmy<0)xmy+=sp; if(xmy<chWt)hatch=true; }
    ...
    sketch: function(d,W,H,p,t){ var a=FM.evalProp(p.amount,t); if(a==null)a=0.85; if(a<0)a=0; if(a>1)a=1; var s=d.slice();
      ...
      // Sobel leaves across smooth areas — every gentle gradient produced a faint smear, and there was
      // no way to say "that is not a line". TOOTH gives the paper some grain instead of flat 255 white;
      // it is scaled by how light the pixel already is, so it textures the paper and not the strokes.
      var skDark=p.darkness==null?510:FM.evalProp(p.darkness,t); if(skDark<100)skDark=100; if(skDark>1200)skDark=1200;
      var skThrP=p.threshold==null?0:FM.evalProp(p.threshold,t); if(skThrP<0)skThrP=0; if(skThrP>100)skThrP=100;
      var skThr=skThrP/400;
      var skTooth=p.tooth==null?0:FM.evalProp(p.tooth,t); if(skTooth<0)skTooth=0; if(skTooth>100)skTooth=100; function lum(xx,yy){ var j=(yy*W+xx)*4; return s[j]*0.299+s[j+1]*0.587+s[j+2]*0.114; } for(var y=0;y<H;y++){ for(var x=0;x<W;x++){ var i=(y*W+x)*4; if(d[i+3]===0)continue; var xm=x>0?x-1:0, xp=x<W-1?x+1:W-1, ym=y>0?y-1:0, yp=y<H-1?y+1:H-1; var gx=(lum(xp,ym)+2*lum(xp,y)+lum(xp,yp))-(lum(xm,ym)+2*lum(xm,y)+lum(xm,yp)); var gy=(lum(xm,yp)+2*lum(x,yp)+lum(xp,yp))-(lum(xm,ym)+2*lum(x,ym)+lum(xp,ym)); var mag=Math.sqrt(gx*gx+gy*gy)/1442; if(mag>1)mag=1;
      if(skThr>0&&mag<skThr)mag=0;
      var v=255-mag*skDark; if(v<0)v=0;
      if(skTooth>0){ var skH=(x*374761393+y*668265263)|0; skH=(skH^(skH>>>13))*1274126177|0; skH=skH^(skH>>>16);
  ...
  function drawWarpEffect(ctx, layer, t, scene, fx, mapFn) {
    ...
    if (opacity <= 0) return;
    const proj = (scene && scene.project) || { width: ctx.canvas.width, height: ctx.canvas.height };
    const PW = proj.width, PH = proj.height;   // see plateScale — 1 for export/1:1, smaller for a reduced preview
    // Covers what the TARGET covers, not the comp — see nestedPlate (queue 323).
    const _np = nestedPlate(ctx, proj), ps = _np.ps, OX = _np.OX, OY = _np.OY;
    const W = _np.W, H = _np.H, PWp = _np.PWp, PHp = _np.PHp;
    const d = _wpDepth++;
  ...
  function drawSquish(ctx, layer, t, scene, fx) {
    ...
    /* ---- ROOM FOR EFFECTS THAT MOVE THE PIXELS (queue 323) --------------------------------------
     * The last of THREE things that each had to be right before an effect-driven move could squash,
     * and each of which, alone, changed nothing — which is a good way to conclude the wrong thing
     * twice, and did:
     *  1. the effect plates had to stop being comp-sized (see nestedPlate);
     *  2. Squish had to composite OUTERMOST rather than innermost, so it measures where the layer ends
     *     up rather than where it started (see the pin in drawLayer);
     *  3. and THIS box — which decides whether a wall is worth looking at at all, and how much padding
     *     to cut — is built from the transform MATRIX. That knows about parenting, behaviours and the
     *     camera, and nothing about an effect displacing pixels at render time. A shake that pushes the
     *     layer over a wall leaves the matrix mid-frame, so the pass early-outs and no padded plate is
     *     ever built for the other two fixes to work in.
     * A BOUND, NOT A MEASUREMENT. There is no way to ask an arbitrary effect how far it will move
     * things, so the layer's own half-extent is the allowance: generous enough for the shakes and
     * wiggles anyone reaches for (they are tuned relative to the thing they shake) and bounded, so the
     * plate cannot grow without limit. Anything beyond it is still clipped exactly as today — this
     * makes the common case work rather than claiming to solve the general one.
     * Only when the layer HAS another effect, so a layer with just Squish pays nothing. */
    /* NAMED, NOT "ANY EFFECT". The first version padded whenever the layer carried anything else, and
     * that broke `[squish, bulge]`: a bulge does not move the layer, it redistributes it, and handing
     * it a much larger plate changed the geometry it warps — Squish then found nothing at the wall and
     * a case that worked stopped working. The list is the effects that displace the whole layer, which
     * is precisely what he described ("a shake effect or any effect that makes the object move"); an
     * effect that only rearranges pixels inside the layer's own box needs no extra room, because the
     * box is where it already is. A new mover added to the app has to be added here — the alternative
     * is padding for everything, and that is the version that broke something. */
    const _hasMover = (layer.effects || []).some(e => e && e !== fx && e.enabled !== false && MOVER_FX[e.type]);
    const fxSlack = (msc) => (_hasMover ? 0.5 * Math.max(sz.w || 0, sz.h || 0) * (msc || 1) : 0);
    ...
    if (Mx) {
      ...
      const slack = strokeW * msc * 0.75 + 4 + fxSlack(msc);
      ...
      const reach = 0.5 * Math.hypot(sz.w || 0, sz.h || 0) * sc + strokeW * sc + 4 + fxSlack(sc);
  ...
  function drawCompoundBlur(ctx, layer, t, scene, fx) {
    ...
    if (opacity <= 0) return;
    const proj = (scene && scene.project) || { width: ctx.canvas.width, height: ctx.canvas.height };
    const PW = proj.width, PH = proj.height;   // see dspSlot
    // Covers what the TARGET covers, not the comp — see nestedPlate (queue 323).
    const _np = nestedPlate(ctx, proj), ps = _np.ps, OX = _np.OX, OY = _np.OY;
    const W = _np.W, H = _np.H, PWp = _np.PWp, PHp = _np.PHp;
    const clean = Object.assign({}, layer, { effects: (layer.effects || []).filter(e => e !== fx) });
    ...
    try {
      ...
      ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
      ctx.filter = 'none';
      ctx.drawImage(slot.B, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(B,0,0) at scale 1
      ctx.restore();
    } finally { _dspLvl--; }
  }
  ...
  function drawDisplaceEffect(ctx, layer, t, scene, fx, polar) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    ctx.drawImage(_dspB, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(B,0,0) at scale 1
    ctx.restore();
    } finally { _dspLvl--; }
  }
  ...
  function drawCanvasEffect(ctx, layer, t, scene, fx, fn) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    ctx.drawImage(_cfB, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(_cfB,0,0) at scale 1
    ctx.restore();
    } finally { _cfDepth--; }
  }
  ...
  function draw3DTiltLayer(ctx, layer, t, scene) {
    ...
    _t3A.__fmRS = ps; _t3A.__fmOX = OX; _t3A.__fmOY = OY;   // the nested drawLayer renders through baseT
    ...
    ctx.save();
    baseT(ctx);
    if (ctx.canvas === _t3A) ctx.clearRect(OX, OY, PWp, PHp);   // paranoia: never composite over our own plate scratch
    ctx.globalAlpha = opacity;
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    ctx.drawImage(_t3B, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(B,0,0) at scale 1
    ctx.restore();
    } finally { _t3Depth--; }
  }
  ...
  function emitterWorldPos(layer, bt, cx, cy) {
    ...
    while (pid && scene && !seen.has(pid) && depth++ < 16) {
      seen.add(pid);
      const pl = FM.clipAt(scene, pid, bt);   // the half that actually covers bt, if this parent was split
  ...
  function drawRgbSplit(ctx, layer, t, scene, d, fx) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (ctx.canvas === _rgbA) ctx.clearRect(OX, OY, PWp, PHp);   // two stacked RGB Splits: clear our own scratch so B replaces, not double-composites (see drawTint)
    ctx.drawImage(_rgbB, OX, OY, PWp, PHp);   // plate → project units; identical to drawImage(B,0,0) at scale 1
    ctx.restore();
  }
  ...
  function drawPosterize(ctx, layer, t, scene, levels, fx) {
    ...
    if (opacity <= 0) return;
    const P = (scene && scene.project) || { width: ctx.canvas.width, height: ctx.canvas.height };
    const PW = P.width, PH = P.height;   // see plateScale
    // Covers what the TARGET covers, not the comp — see nestedPlate (queue 323).
    const _np = nestedPlate(ctx, P), ps = _np.ps, OX = _np.OX, OY = _np.OY;
    const W = _np.W, H = _np.H, PWp = _np.PWp, PHp = _np.PHp;
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (ctx.canvas === _psA) ctx.clearRect(OX, OY, PWp, PHp);   // two stacked Posterizes: clear our own scratch so B replaces, not double-composites (see drawTint)
    ctx.drawImage(_psB, OX, OY, PWp, PHp);   // plate → project units; identical at scale 1
    ctx.restore();
  }
  ...
  function drawTint(ctx, layer, t, scene, amount, colorHex, fx) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (ctx.canvas === _tiA) ctx.clearRect(OX, OY, PWp, PHp);   // two stacked Tints: ctx IS our scratch A (holds the clean layer) → clear so B replaces instead of double-compositing the edges
    ctx.drawImage(_tiB, OX, OY, PWp, PHp);
    ctx.restore();
  }
  ...
  function drawThreshold(ctx, layer, t, scene, level, fx) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (ctx.canvas === _thA) ctx.clearRect(OX, OY, PWp, PHp);   // two stacked Thresholds: clear our own scratch so B replaces, not double-composites (see drawTint)
    ctx.drawImage(_thB, OX, OY, PWp, PHp);   // plate → project units; identical at scale 1
    ctx.restore();
  }
  ...
  function drawDuotone(ctx, layer, t, scene, amount, shadowHex, hiHex, fx) {
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (ctx.canvas === _duA) ctx.clearRect(OX, OY, PWp, PHp);   // two stacked Duotones: clear our own scratch so B replaces, not double-composites (see drawTint)
    ctx.drawImage(_duB, OX, OY, PWp, PHp);   // plate → project units; identical at scale 1
    ctx.restore();
  }
  ...
  function drawPixelate(ctx, layer, t, scene, size, fx) {
    ...
    if (opacity <= 0) return;
    const P = (scene && scene.project) || { width: ctx.canvas.width, height: ctx.canvas.height };
    /* Covers what the TARGET covers — see nestedPlate (queue 323). This plate works in PROJECT units
       rather than plate pixels (the block size below is a project length, and the mosaic is meant to be
       comp-resolution whatever the preview scale is), so the region is taken in those units too and the
       origin is carried on the canvas for baseT to apply. */
    const _np = nestedPlate(ctx, P);
    const OX = _np.OX, OY = _np.OY;
    const W = Math.max(1, Math.round(_np.PWp)), H = Math.max(1, Math.round(_np.PHp));
    ...
    baseT(actx); actx.clearRect(OX, OY, W, H);
    actx.globalAlpha = 1; actx.globalCompositeOperation = 'source-over'; actx.filter = 'none';
    ...
    ctx.globalCompositeOperation = BLEND[layer.blendMode] || 'source-over';
    ctx.filter = 'none';
    if (size <= 1) { ctx.drawImage(_pxA, OX, OY, W, H); ctx.restore(); return; }
    ...
    ctx.drawImage(_pxS, 0, 0, sw, sh, OX, OY, W, H);      // upscale → blocky (or soft)
  ...
  FM.SHAPE_POLYS = (function () {
    ...
    // snowflake: ONE arm (center→tip + two tip-ward branches) rotated 6×60° — symmetric by
    // construction. The old build reused the TOP arm's branch geometry unmirrored at the bottom
    // of each spoke, so half the branches pointed the wrong way (read as two flakes overlaid).
    (function(){
      const bar=(x0,y0,x1,y1,w)=>{ const dx=x1-x0,dy=y1-y0,L=Math.hypot(dx,dy),nx=-dy/L*w/2,ny=dx/L*w/2;
        return [[x0+nx,y0+ny],[x1+nx,y1+ny],[x1-nx,y1-ny],[x0-nx,y0-ny]].map(p=>[r4(p[0]),r4(p[1])]); };
      const arm=[bar(0.5,0.5,0.5,0.055,0.055), bar(0.5,0.26,0.355,0.115,0.048), bar(0.5,0.26,0.645,0.115,0.048)];
      const polys=[]; for(let k=0;k<6;k++){ const a=k*PI/3; arm.forEach(q=>polys.push(rot(q,0.5,0.5,a))); }
      S.snowflake=polys;
    })();
  ...
  FM.layerSize = function (layer) {
    ...
    if (layer.type === 'text') {
      // ONE measuring context for the life of the page, not one per call. layerSize() is called from
      // canvas-edit's update(), which render() calls every frame, so a selected text layer was
      // allocating a fresh canvas AND a fresh 2D context 60 times a second — measured at exactly
      // 1.00 canvases per rendered frame with a text layer selected, 0.00 with a shape. It is
      // garbage rather than a leak, but it is garbage generated by the frame loop. Same singleton
