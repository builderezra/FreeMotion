...
(function (FM) {
  ...
  function buildDigest() {
    return [
      ...
      'ROTATION is in DEGREES (clockwise). Z-ORDER: lower z = nearer the TOP/front; z 0 is frontmost. Omit z and a new layer goes on TOP of everything already there, so layers listed background-first stack the way they read.',
      'TRANSFORM channels (setProp transform.* or keyframe any of them): x, y, scale (uniform), scaleX/scaleY (non-uniform multipliers on scale, e.g. squash & stretch), rotation(deg), skewX/skewY(deg, -80..80), z (planar DEPTH: +z recedes/shrinks, -z approaches/enlarges), opacity(0..1). anchorX/anchorY(0..1) can be SET but never keyframed. setProp on a channel that already has keyframes moves its WHOLE animation so it shows that value in the frame you were shown (the motion is kept); addKeyframe replaces the motion.',
