...
- [x] **Clear the rest of the BUG-HUNT backlog — EMPTY as of v9.41.** Every open finding in that file is now struck. It stood at 21 when this session started.
      ...
      *Progress 17 Aug:* a **seventeenth** struck off — **the layer-visibility eye being a 15px tap
      ...
      than fixed: the floating + button covering the row-reorder handles — that button no longer
      renders since the Add-layer row replaced it, re-measured to be sure.
      ...
      *Progress 17 Aug:* a **fifteenth** struck off — **a copied keyframe pasting into a completely
      ...
      it — or invented a parameter that effect does not have, silently, into your project file.
...
- [x] **365 — The camera button in the view rail: tap adds, tap hides, tap unhides, hold opens its
      settings.** ✅ **v10.12, all four clauses** — plus the button now shows which of the three states it
      is in, because a state machine you cannot see is worse than a button that does one thing. (18 Aug.) His words, verbatim: *"Also the camera button in the view menu works like
      ...
      **The spec, as a state machine:**
      1. [x] ✅ **Tap** with no camera in the project → **add** one.
      2. [x] ✅ **Tap** with a camera present and visible → **hide** it.
      3. [x] ✅ **Tap** again → **unhide** it. (So after the first tap it is a two-state toggle, and it never
             deletes the camera — nothing here removes one.)
      4. [x] ✅ **Hold** → open the camera's settings (and the click ending the hold is swallowed).
